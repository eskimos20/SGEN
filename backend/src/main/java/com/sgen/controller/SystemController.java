...
public class SystemController {
    ...
    private Map<String, Object> getMemoryInfo() {
        ...
        try {
            ...
            log.warn("Failed to read /proc/meminfo", e);
        }
        
        long total = memTotal;
        long free = memAvailable > 0 ? memAvailable : (memFree + buffers + cached);
        ...
        memory.put("usagePercent", total > 0 
            ? ((double) used / total) * 100 
            : 0);
        
        Runtime runtime = Runtime.getRuntime();
        memory.put("jvmTotal", runtime.totalMemory());
        memory.put("jvmFree", runtime.freeMemory());
        memory.put("jvmMax", runtime.maxMemory());
