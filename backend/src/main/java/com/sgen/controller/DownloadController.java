...
public class DownloadController {
    ...
    /**
     * Check if APK is available either as classpath resource or file
     */
    private boolean isApkAvailable() {
        // Prefer the APK packaged in the classpath (JAR or target/classes)
        ...
        // Fall back to file system resource
    ...
    private Resource getApkResource() {
        ...
        ClassPathResource classpathResource = new ClassPathResource("static/downloads/sgen-android.apk");
        if (classpathResource.exists()) {
            log.info("Serving APK from classpath");
        ...
        Path apkPath = getApkFilePath();
        if (apkPath.toFile().exists()) {
            log.info("Serving APK from file: {}", apkPath);
