      ...
      if (activity.icu_intervals && Array.isArray(activity.icu_intervals)) {
        const intervals = activity.icu_intervals
          .filter(interval => {
            ...
            // Allow recovery/rest intervals even if they report zero watts
            const type = (interval.type || '').toUpperCase();
            if (type === 'RECOVERY' || type === 'REST') return true;
            // For work intervals, watts data must be > 0 if present; intervals without watts are still included
