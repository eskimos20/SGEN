...
const WeatherSearch = ({ openMeteoEnabled, onToggleOpenMeteo, onSelect, onCancel }) => {
    ...
    return (
        <div className="space-y-3">
            <div className="flex items-center justify-between p-3 border border-gray-200 rounded-lg bg-gray-50">
                <div>
                    <div className="font-medium text-sm text-gray-900">Enable OpenMeteo API</div>
                    <div className="text-xs text-gray-500">
                        When enabled, weather and search use OpenMeteo instead of SMHI.
                    </div>
                </div>
                <button
                    onClick={onToggleOpenMeteo}
                    className={`
                        relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none
                        ${openMeteoEnabled ? 'bg-blue-600' : 'bg-gray-300'}
                    `}
                    aria-pressed={openMeteoEnabled}
                    aria-label="Enable OpenMeteo API"
                    type="button"
                >
                    <span
                        className={`
                            inline-block h-4 w-4 transform rounded-full bg-white transition-transform
                            ${openMeteoEnabled ? 'translate-x-6' : 'translate-x-1'}
                        `}
                    />
                </button>
            </div>
            ...
            <div className="relative">
                <input
                    type="text"
                    placeholder={openMeteoEnabled ? 'Search city (e.g. Stockholm)...' : 'Search SMHI station (e.g. Luleå)...'}
                    value={query}
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-xs text-gray-500">
                                            ...
                                            {station.active === false && <span className="ml-2 text-amber-500">inactive</span>}
                                        </div>
                                    </div>
                                </div>
                            </button>
