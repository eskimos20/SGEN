...
import WeatherRadarMap from './weather/WeatherRadarMap';

const WeatherWidget = () => {
    const { forecasts, userLocation, openMeteoEnabled, loading, error, saveLocation, toggleOpenMeteo } = useWeather();
            ...
            <div className="flex-1">
                <div className="flex items-center gap-2">
                    ...
                    <span
                        ...
                        title={openMeteoEnabled ? 'OpenMeteo API is active' : 'SMHI API is active'}
                    >
                        {openMeteoEnabled ? 'OpenMeteo' : 'SMHI'}
                    </span>
                </div>
                {userLocation && (
    ...
    if (error && forecasts.length === 0) {
        return (
            <div className="bg-white rounded-xl sm:shadow-sm border border-gray-200 p-3 sm:p-6">
                ...
                {showSettings ? (
                    <WeatherSearch
                    openMeteoEnabled={openMeteoEnabled}
                    onToggleOpenMeteo={toggleOpenMeteo}
        ...
        <div className="bg-white rounded-xl sm:shadow-sm border border-gray-200 p-3 sm:p-6">
            ...
            {forecasts.length > 0 ? (
                <div className="overflow-x-auto pb-2">
                    <div className="flex gap-3 min-w-max sm:min-w-0 sm:grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                        {forecasts.slice(0, openMeteoEnabled ? 8 : 7).map((day, idx) => (
            ...
            {userLocation && (
                <div className="mt-6 pt-6 border-t border-gray-200">
                    <h3 className="text-sm font-semibold text-gray-900 mb-2">Weather Radar — {userLocation.cityName}</h3>
                    <WeatherRadarMap
