...
import { getSportSettingsForType, DEFAULT_HR_ZONES } from '../../utils/zoneUtils';
...
const AdHocWorkoutPlanner = ({ sportSettings, weightKg, onKcalEstimated }) => {
  ...
  // Sport-specific HR zones for non-FTP sports
  const currentSportSettings = useMemo(() =>
    getSportSettingsForType(sportSettings, currentSport?.key),
    [sportSettings, currentSport]
  );
  const currentSportZones = useMemo(() =>
    currentSportSettings?.hrZones || DEFAULT_HR_ZONES,
    [currentSportSettings]
  );
  ...
  const intensityFactor = hasPower
    ? Math.min(1, Math.max(0, intensityPercent / 100))
  ...
  const macros = useMemo(() => {
    ...
    const isHighIntensity = hasPower ? intensityPercent >= 88 : zoneMetrics.effortLevel === 'hard';
  ...
  }, [estimatedKcal, weightKg, durationMinutes, zoneValue, hasPower, intensityFactor, intensityPercent, zoneMetrics]);
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      ...
      <div className="p-3 sm:p-4 space-y-4">
        ...
        {hasPower ? (
          ...
          <div>
            <label className="text-sm font-medium text-gray-700 mb-2 block">
              Intensity: {zoneMetrics.zoneLabel} — {zoneMetrics.zoneName} (MET {zoneMetrics.met})
            </label>
            <input
              type="range"
              min="1"
              max={currentSportZones.length}
              step="0.1"
              value={zoneValue}
            ...
            <div className="flex justify-between text-[10px] sm:text-xs text-gray-400 mt-1">
              {currentSportZones.map((z, i) => (
                <span key={z.name} className={Math.round(zoneValue) === i + 1 ? 'text-indigo-600 font-semibold' : ''}>
                  {z.name}
                </span>
              ))}
            </div>
