    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="space-y-4">
        ...
        {dayItems.map((item, idx) => {
          ...
          // Derive intensity info from icu_intensity when available, fallback to calculated metrics for planned workouts
          const rawIF = item.icu_intensity || plannedMetrics?.icu_intensity || 0;
          const normalizedIF = rawIF > 2 ? rawIF : rawIF * 100; // normalize to percentage (e.g. 65)
          // Use normalized IF % directly as 0-1 intensity factor (e.g. 56% -> 0.56)
          const activityIF = normalizedIF > 0 ? Math.min(1, Math.max(0, normalizedIF / 100)) : null;
