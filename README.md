...
### Advanced Features
- **Weather Integration**: Real-time weather forecasts via SMHI or OpenMeteo — up to 10 days of data including precipitation, wind, cloud cover, and thunderstorm probability. Users can toggle between SMHI (Swedish stations) and OpenMeteo (global city search) per account.
- **Radar & Satellite Map**: Live radar/satellite animation on the dashboard via LibreWXR, with smooth crossfade transitions and support for both SMHI and OpenMeteo locations.
...
- **Build Tool**: Maven
- **External APIs**: Intervals.icu, OpenAI, SMHI (weather), OpenMeteo (global weather & geocoding), LibreWXR (radar & satellite tiles)
...
- `GET /api/weather/forecast` - Get weather forecast (SMHI or OpenMeteo, based on user setting)
- `GET /api/weather/location` - Get user's saved weather location
- `POST /api/weather/location` - Save weather location
- `GET /api/weather/search/{cityName}` - Search weather location (SMHI station or OpenMeteo city)
- `GET /api/weather/settings` - Get weather settings (OpenMeteo enabled status)
- `POST /api/weather/settings` - Save weather settings (enable/disable OpenMeteo)
...
The dashboard includes a weather widget and a live radar/satellite map. Weather and radar data can be fetched from either SMHI or OpenMeteo, depending on each user's preference.

### Weather Forecasts

- **SMHI**: Swedish local forecasts from the Swedish Meteorological and Hydrological Institute. Search for Swedish measurement stations and get up to 10 days of weather data.
- **OpenMeteo**: Global forecasts with world-wide city search (including correct UTF-8 handling of characters such as å, ä and ö). Toggle between SMHI and OpenMeteo in the weather settings; the rest of the UI stays visually identical.
- A badge next to the **Weather Forecast** heading shows which source is currently active.

### Radar & Satellite Map

- The dashboard shows a live radar/satellite animation based on the selected weather location.
- Data is served from the **LibreWXR** API (`https://api.librewxr.net` by default, configurable via the `VITE_LIBREWXR_API_URL` environment variable).
- The map supports smooth crossfade transitions between frames, resets when the weather station/city changes, and is constrained to its own stacking context to avoid overlapping mobile UI controls.
- On deploy, `deploy-to-server.sh` can automatically pull the latest LibreWXR changes and rebuild the Docker container.
